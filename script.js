...
function drawFallback(){
  ...
  (function loop(){
    ...
    for(let i=0;i<bars;i++){
      const x = (i/(bars-1)) * (W * 1.0);
      const w = W/bars*1.0; // estirado para tope a tope
...
function startMainViz(){
  ...
  (function loop(){
    ...
    const usefulLen = Math.floor(buf.length * 0.7);
    const useful = buf.slice(0, usefulLen);

    const W = canvas.width, H = canvas.height;
    const bars = 40;
    const step = Math.floor(useful.length / bars);
    ...
    for(let i=0;i<bars;i++){
      ...
      for(let j=0;j<step;j++){
        sum += useful[i*step+j] || 0;
      }
      const v = (sum/step)/255;

      const x = (i/(bars-1))*W;
      const w = W/bars*1.2; // estirado para cubrir todo
